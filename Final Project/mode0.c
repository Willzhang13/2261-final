...
int collisionCheck(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // Top Right
    current = collisionMap[OFFSET(col + width + colShift -1, row + rowShift, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    // Bottom Left
    current = collisionMap[OFFSET(col + colShift, row + height + rowShift - 1, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }

    // Bottom Right
    current = collisionMap[OFFSET(col + width + colShift -1 , row + height + rowShift - 1, mapWidth)];
    if (current < smallestSoFar) {
        smallestSoFar = current;
    }
